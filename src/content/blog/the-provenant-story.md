...
- **Source-faithful.** Copyright and license text is reported the way it actually appears in your files — `©` stays `©`, statements aren't silently trimmed — so what Provenant reports is what's really there.
...
- **Every scan emits a provenance record** naming exactly which dataset it used and how that dataset was modified.
